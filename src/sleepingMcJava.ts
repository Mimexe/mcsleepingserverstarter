...
export class SleepingMcJava implements ISleepingServer {
  ...
  init = async () => {
    ...
    this.server.on("connection", (client: Client) => {
      this.logger.info(
        `A Prince has taken a quick peek. [${client.version}_${client.socket.remoteAddress}]`
      );
    });
    ...
    this.server.on("login", (client) => {
      ...
      this.logger.info(
        `Prince [${userName}.${client.state}_${client.socket.remoteAddress}] has come, time to wake up.`
      );
